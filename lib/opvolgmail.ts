 ...
 * richtprijs, en als slot het voorproefje-aanbod (trede 1 naar 2 van het trappetje).
...
function klantTaal(regel: string): string | null {
  ...
  if (/geen https/i.test(regel))
    return "Je site heeft geen slotje (https), waardoor browsers hem markeren als 'niet veilig'.";
...
export function maakOpvolgmail(
  ...
  const prijsRegel = prijs
    ? `Mijn voorstel: voor een site van jouw omvang kost het overzetten eenmalig ${prijs}, daarna €19 per maand voor hosting, beheer en aanpassingen via de chat.`
    : `Mijn voorstel: het overzetten kost eenmalig vanaf €150 (afhankelijk van de omvang van je site), daarna €19 per maand voor hosting, beheer en aanpassingen via de chat.`;
