...
- Verander GEEN teksten of URL-paden; alleen vormgeving en structuur. Uitzonderingen: (a) dode interne links (naar /tag/, /category/ of andere niet-gebouwde pagina's) repareer je door ze naar het blogoverzicht te laten wijzen of er platte tekst van te maken; (b) placeholder-tekst (lorem ipsum, Engelse thema-restanten op een Nederlandse site) verwijder je alsnog, inclusief de sectie als die verder leeg is; (c) overzichtsblokken (diensten, team, artikelen) waarvan items niet doorklikken naar hun detailpagina maak je alsnog klikbaar.
...
Instructies:
- DE NORM IS: EXACT OVERZETTEN. De nieuwe site is dezelfde site. Zelfde indeling, zelfde beelden,
  zelfde teksten, zelfde kleuren, zelfde beweging, zelfde gedrag. Je maakt geen eigen
  ontwerpkeuzes en je vereenvoudigt niets. Werkt iets alleen dankzij een WordPress-plugin, dan
  bouw je het na zodat het er hetzelfde uitziet en zich hetzelfde gedraagt; je vervangt het niet
  door iets kleiners. Kan iets écht niet zonder server, noteer het dan in ontbrekende-media.txt
  in plaats van er stilletjes iets simpelers voor in de plaats te zetten. Alles wat de bezoeker
  zag of merkte en wat er niet meer is, is verlies, ook als er geen foutmelding van komt.
...
- SLIDERS (de echte beelden staan in de afbeeldingen-kaart; nooit leeg of nagemaakt). Volgens de
  norm hierboven: een slider die beweegt, beweegt ook op de nieuwe site.
  - Hero-/fotoslider: bouw de binnenkomst na met CSS-keyframes en getrapte vertragingen, zoals
    ...
    de echte beelden. Kijk af bij evc-autotechniek (hero-omlaag, hero-links, hero-open).
    Statisch alleen als er ook op de oude site maar één slide was, en de tekst van élke slide
    blijft staan, desnoods als sectie eronder.
...
- Maak één gedeeld stijlblad site/stijl.css: rustig, professioneel, passend bij het type bedrijf. Zet er altijd [hidden]{display:none!important} in: een klasse met display:grid of flex wint anders van het hidden-attribuut, en dan werken filters die el.hidden zetten niet. Een <a> met button of btn in de klasse is een knop en krijgt de knopstijl van de site, nooit een kale tekstlink. Mobielvriendelijk (viewport-meta, geen vaste breedtes, leesbare tekst, aantikbare knoppen, hamburger-menu bij veel menu-items).
...
- FORMULIEREN: bouw elk contactformulier van de oude site na als statisch formulier met dezelfde velden, in de stijl van de site, met method="POST" en action="https://wordswap.nl/api/formulier", een verborgen input name="_site" value="${repoNaam}", een verborgen input name="_formulier" met een korte kebab-case naam die het DOEL van dat formulier beschrijft en per doel uniek is binnen de site (bv. "contact", "offerte", "terugbellen", "sollicitatie", "nieuwsbrief-aanmelding"; twee formulieren met hetzelfde doel op verschillende pagina's mogen dezelfde naam delen), en een verborgen honeypot-veld name="_extra" (leeg laten, via CSS verborgen). Die _formulier-naam is nodig: daarop worden de bevestigingsmails per formulier geregeld. LET OP, formulieren van EXTERNE DIENSTEN zijn de uitzondering: een insluitcode van ActiveCampaign, Mailchimp, MailerLite, Laposta, Calendly en vergelijkbare diensten (script, iframe of div met embed-code) neem je 1-op-1 over en bouw je NOOIT om naar ons endpoint — dan breken hun opvolgmails, lijsten en automatiseringen. Alleen formulieren die de oude site zelf verwerkte (CF7, WPForms, Gravity Forms, eigen PHP) bouw je na op ons endpoint.
