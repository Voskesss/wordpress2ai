...
/**
 * Eén chatbeurt: bericht van de eigenaar → AI past de site aan → concept.
 * Gedeeld door de portaalchat (app/api/chat) en het WhatsApp-kanaal, zodat
 * er maar één pijplijn is met hetzelfde slot, budget en concept.
 */

// Moet gelijk zijn aan maxDuration van app/api/chat/route.ts. De tijdbewaker
// hieronder stopt de agent zelf ruim vóór die harde grens.
const maxDuration = 800;

export type ChatBeurtOpties = {
  /** Mag deze gebruiker als beheerder bij elke site? Alleen aangeroepen als
   * hij geen eigenaar is of de site gepauzeerd staat. */
  isBeheerder: () => Promise<boolean>;
  /** Uiterste tijdstip (ms) waarop de beurt klaar moet zijn, als de
   * aanroeper al tijd van zijn eigen functieduur heeft verbruikt. */
  eindtijd?: number;
  /** Via welk kanaal het bericht binnenkwam; bepaalt uitleg aan de AI. */
  kanaal?: "portaal" | "whatsapp";
};

const WHATSAPP_REGEL = `De eigenaar stuurt dit bericht via WhatsApp, niet via het portaal. Hij ziet dus GEEN websitevoorbeeld, gele conceptbalk of andere portaalknoppen — noem die niet. Na jouw antwoord krijgt hij automatisch een link om het concept te bekijken en knoppen "Publiceren" en "Weggooien" in WhatsApp; dat hoef je niet te zeggen. Houd je antwoord extra kort (het leest op een telefoon). Een KEUZES-regel mag gewoon: die verschijnt als keuzelijst in WhatsApp.`;
...
/** Nette, unieke bestandsnaam voor een meegestuurde pdf (bestanden/<naam>.pdf). */
export function documentNaam(origineel: string, gebruikt: Set<string>) {
  const basis =
    origineel
      .replace(/\.pdf$/i, "")
      .normalize("NFKD")
  ...
  let naam = basis;
  let n = 2;
  while (gebruikt.has(naam)) naam = `${basis}-${n++}`;
  gebruikt.add(naam);
  return `${naam}.pdf`;
}
...
export async function voerChatBeurtUit(
  req: Request,
  userId: string,
  opties: ChatBeurtOpties,
) {
  const isBeheerder = opties.isBeheerder;
  let siteId: number;
  let bericht: string;
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    // Pdf's: de chat stuurde ze al mee als "document", maar ze werden hier
    // nooit uitgelezen — ze verdwenen stilletjes.
    const docs = form
      .getAll("document")
      .filter((f): f is File => f instanceof File && f.size > 0);
    ...
    }
    {
      ...
      for (const doc of docs) {
        if (!/\.pdf$/i.test(doc.name) && doc.type !== "application/pdf") {
          return NextResponse.json(
            { error: `${doc.name} is geen pdf. Alleen pdf-documenten kunnen mee.` },
            { status: 400 },
          );
        }
        if (doc.size > MAX_DOC_BYTES) {
          return NextResponse.json(
            { error: `Document ${doc.name} is te groot (max ${MAX_DOC_BYTES / 1024 / 1024} MB)` },
            { status: 400 },
          );
        }
        ...
        if (data.subarray(0, 5).toString("latin1") !== "%PDF-") {
          return NextResponse.json(
            { error: `${doc.name} lijkt geen geldige pdf.` },
            { status: 400 },
          );
        }
        documenten.push({
          naam: `${DOCUMENTEN_MAP}/${documentNaam(doc.name, gebruikteDocNamen)}`,
          data,
...
4. Kan het document persoonsgegevens van anderen bevatten (een ingevuld formulier, een lijst met namen, een cv, een offerte)? Waarschuw dan kort dat alles op de site voor iedereen te downloaden is, en vraag of hij dat zeker weet vóór je het plaatst.`
            ...
            openConcept
              ...
              : null,
            controleRegel,
            opties.kanaal === "whatsapp" ? WHATSAPP_REGEL : null,
          ...
          const wekker = setTimeout(
            ...
            Math.max(
              30_000,
              Math.min(
                (maxDuration - 80) * 1000 - (Date.now() - klok),
                // Aanroeper met eigen deadline (WhatsApp): ook dáár ruim vóór stoppen
                opties.eindtijd ? opties.eindtijd - Date.now() - 80_000 : Infinity,
              ),
            ),
          );
