...
export default async function AfspraakPagina({ params }: { params: Promise<{ token: string }> }) {
  ...
  const aangevraagd = stand?.afspraken.find((a) => a.status === "aangevraagd");
  const dagen: Dag[] = stand
    ? vrijeMomenten(stand.blokken, await bezetteTijden(), new Date()).map((d) => ({
        datum: d.datum,
    ...
    <main className="mx-auto max-w-2xl px-6 py-14">
      ...
      {!site ? (
        ...
        <p className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-stone-800">
          Er staat al een afspraak: <strong>{momentInWoorden(bevestigd.start, bevestigd.duurMinuten)}</strong>. Komt het
          toch niet uit? Mail Jos even, dan zoeken we een ander moment.
        </p>
      ) : aangevraagd ? (
