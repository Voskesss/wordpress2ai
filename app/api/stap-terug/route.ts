...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (rij.change.status !== "concept") {
      return NextResponse.json(
        { error: "Alleen bij een openstaand concept" },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          error: "leeg",
          melding:
            "Er valt niets meer terug te draaien in dit concept — gebruik Verwijder om het hele concept weg te doen.",
        },
        { status: 400 },
      );
    }
    ...
    if (
      rij.site.isDemo &&
      rij.change.baseSha &&
      kop.sha === rij.change.baseSha
    ) {
      return NextResponse.json(
        {
          error: "leeg",
          melding:
            "Je bent terug bij het begin van dit concept — gebruik Verwijder om het hele concept weg te doen.",
        },
        { status: 400 },
      );
    }
    ...
    if (!vorige) {
      return NextResponse.json(
        { error: "Geen eerdere stap gevonden" },
        { status: 400 },
      );
    }
    ...
    await db
      ...
      .set({
        promptTekst:
          `${rij.change.promptTekst} → (laatste stap teruggedraaid)`.slice(
            0,
            500,
          ),
      })
