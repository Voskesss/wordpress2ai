        ...
        try {
          ...
          const contextRegels = [
            ...
            selectie
              ...
              : null,
            videoPaden
              ? `De eigenaar heeft een VIDEO meegestuurd; die is al gecomprimeerd voor het web en staat op ${videoPaden.video}${videoPaden.poster ? ` met poster-afbeelding ${videoPaden.poster}` : ""}. Plaats hem waar het bericht om vraagt. BEKIJK eerst de poster (lees_bestand) om de verhouding te zien: een staande (9:16) video zet je nooit stilzwijgend in een liggend of bijna-vierkant kader met object-fit: cover — pas het kader aan of waarschuw kort met een KEUZES-regel. Als achtergrond/hero-video: <video autoplay muted loop playsinline preload="metadata"${videoPaden.poster ? ` poster="/${videoPaden.poster}"` : ""}> met <source src="/${videoPaden.video}" type="video/mp4">, netjes gepositioneerd achter de tekst, en respecteer prefers-reduced-motion (dan alleen de poster). Als gewone video op een pagina: <video controls preload="metadata" poster=...>. Verwijder een eventuele oude hero-video-verwijzing die hij vervangt, maar laat het oude bestand staan.`
              : null,
