...
function maakPadenAbsoluut(html: string, oudeMap: string): string {
  ...
  const absoluut = (waarde: string) => {
    const w = waarde.trim();
    if (!w || /^(https?:|\/\/|\/|#|mailto:|tel:|data:|javascript:)/i.test(w)) {
      return waarde;
    }
    // ../ en ./ netjes oplossen
  ...
  return html.replace(
    ...
    (heel, attr, quote, waarde) =>
      `${attr}=${quote}${absoluut(waarde)}${quote}`,
  );
}
...
export async function GET(req: Request) {
  ...
  const branch =
    openConcept?.branch ?? (site.isDemo ? demoBranch(userId) : undefined);
  ...
  try {
    ...
    for (const kandidaat of bestandVoorPad(pad)) {
      const inhoud = await readFile(
        path.join(werkmap, kandidaat),
        "utf8",
      ).catch(() => null);
  ...
  try {
    ...
    try {
      werkmap = openConcept?.branch
        ...
        : eigenBranch
          ? await laadWerkmap(site.githubRepo, eigenBranch).catch(() =>
              laadWerkmap(site.githubRepo),
            )
      ...
      const huidigAdres =
        "/" + oudPad.replace(/index\.html$/i, "").replace(/\.html$/i, "");
      let nieuwPad = oudPad;
      ...
      if (
        ...
        const oudeMap = oudPad.includes("/")
          ? oudPad.slice(0, oudPad.lastIndexOf("/"))
          : "";
        ...
        gewijzigd.push({
          pad: "_redirects",
          inhoud: Buffer.from(nieuweRedirects),
        });
      ...
      if (openConcept) {
        ...
        await db
          ...
          .set({
            ...
            promptTekst:
              `${openConcept.promptTekst} → ${omschrijvingKort}`.slice(0, 500),
          })
      ...
      await db.insert(messages).values([
        {
          siteId: site.id,
          rol: "klant" as const,
          tekst: `[Zelf aangepast] ${omschrijvingKort}`,
          clerkUserId: userId,
        },
        {
          siteId: site.id,
      ...
      return NextResponse.json({
        ...
        bestanden: paden,
        nieuwAdres:
          nieuwPad !== oudPad ? gevraagdAdres.replace(/\/?$/, "/") : undefined,
      });
    } finally {
      if (werkmap) await ruimWerkmapOp(werkmap).catch(() => {});
    }
