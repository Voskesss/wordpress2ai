---
name: migreer-klant
description: Migreer een WordPress-site naar een statische WordSwap-klantsite, volledig vanuit Codex (geen API-pijplijn). Gebruik bij een nieuwe klant-migratie, een WXR/XML-export, of "bouw de site van klant X".
---

# WordPress-klant migreren via Codex
...
```bash
cd ~/wordswap-klanten/<repo> && git init -q 2>/dev/null; git add -A && git commit -q -m "Migratie via Codex"
```
